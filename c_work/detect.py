...
def crop_img(img,dim):
    ...
    h = IntTensor.item(dim[2]) * 1.1#increase by 10 percent to improve Yolo/VGG classification
...
#                            w = IntTensor.item(det_list[3]) * 1.1
                            ...
                            #plot_one_box(xyxy, im0, label=label, color=colors[int(cls)])
                                plot_one_box(det_list, im0, label=label, color = [0,255,0])
                            else:
                                text = "ONLY YOLO3 DETECTION"
                                plot_one_box(det_list, im0, label=label, color = [0, 255, 255])
